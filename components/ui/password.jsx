...
const Password = React.forwardRef(({ className, ...props }, ref) => {
  ...
  return (
    <div className="relative">
      ...
      >
        {showPassword ? (
          <EyeOff className="absolute right-3 top-1/2 h-5 w-6 -translate-y-1/2 transform text-muted-foreground" />
        ) : (
          <Eye className="absolute right-3 top-1/2 h-5 w-6 -translate-y-1/2 transform text-muted-foreground" />
        )}
      </div>
    </div>
  );
